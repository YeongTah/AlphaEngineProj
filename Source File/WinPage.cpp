...
void WinPage_Load()
{
    ...
    if (AEAudioIsValidAudio(sfxWin))
        AEAudioPlay(sfxWin, winGroup, 1.0f, 1.0f, 0);
...
void WinPage_Update()
{
    ...
    {
        for (int i = 0; i < winBtnCount; ++i)
        {
            if (IsAreaClicked(winButtons[i].x, winButtons[i].y,
                winButtons[i].w, winButtons[i].h, mouseX, mouseY))
            {
                if (AEAudioIsValidAudio(sfxButton))
                    AEAudioPlay(sfxButton, winGroup, 1.0f, 1.0f, 0);

                switch (winButtons[i].action)
                {
                case 0: // Level Select
                    Confirmation_Level(GS_WIN, LEVELPAGE, "Are you sure you want to go to Level Select?");
                    next = CONFIRM;
...
void WinPage_Draw()
{
    ...
    for (int i = 0; i < winBtnCount; ++i)
    {
        DrawRect(winButtons[i].x, winButtons[i].y,
            winButtons[i].w, winButtons[i].h,
            0.5f, 0.5f, 0.5f);

        float btnCenterNDCX = winButtons[i].x / 800.0f;
        float btnCenterNDCY = winButtons[i].y / 450.0f;
